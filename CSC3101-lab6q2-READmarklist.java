...
public class READmarklist
{
	public static void main(String args[]) 
 {
	...
	try {
		...
		if(!marklist.exists()) 
			{
			System.out.println("Can not run because there is no txt file!But don't worry:YANG YUKUN has made for u,so please write marks in the txt:"+marklist);
			marklist.createNewFile();
			}
		else if(!resultlist.exists()) 
		{
		System.out.println("Can not run because there is no txt so YANG YUKUN has made a "+resultlist+" for you,you can run again");
		resultlist.createNewFile();
		}
		else 
		{
		System.out.println("All directory and file are exist!Now Start to normally run!Welcome to YANG YUKUN project!");}
		marklist.createNewFile();
		}
	catch(IOException e) {System.out.println("WARNING! Detected IO Exception:\n");e.printStackTrace();}
	catch(Exception error)
	{   
		System.out.println("other errors happened when judged file,Please call YANG YUKUN to solve");
		error.printStackTrace();
	}
	finally {System.out.print("NOW END Reading");}
	//NOW start to read and writer the result of UPM students(such as YANGYUKUN)' mark
	try {
	 ...
	 catch (IOException e) {System.out.println("Detected IO exception when output result!Please contact YANG YUKUN to solve");}
	catch(Exception e) {System.out.println("Detected Other Exception!Please contact YANG YUKUN");e.printStackTrace();}
